...
class Game {
  ...
  snakeAteZero(){
    return  this.snake.head.x === this.food[0].x &&
            this.snake.head.y === this.food[0].y
  }

  snakeAteOne(){
    return this.snake.head.x === this.food[1].x &&
           this.snake.head.y === this.food[1].y
  }

  snakeAteCorrectFood(foodBitEaten){
    return foodBitEaten.toString() === this.currentNumber.nextBit();
  }

  snakeAteFood(){
    if (this.snakeAteZero() && this.snakeAteCorrectFood(0)) {
        this.food[0] = null;
        this.currentNumber.updateBitsToEat();
        this.snake.addSegment();
        console.log("correct food")
    } else if (this.snakeAteOne() && this.snakeAteCorrectFood(1)) {
        this.food[1] = null;
        this.currentNumber.updateBitsToEat();
        this.snake.addSegment();
        console.log("correct food");
    } else if (this.snakeAteZero()) {
  ...
  updateNum(){
    if (this.currentNumber.fullyTranslated()) {
      this.currentNumber = new Num(this.currentNumber.maxNumber * 2);
      console.log(this.currentNumber.decimal, this.currentNumber.binary)
    }
  }
