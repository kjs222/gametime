...
class Snake {
  ...
  isDead() {
    if(this.game.level === 1) {
      return this.head === null || this.hitWall() || this.hitItself();
    } else {
  ...
  hitItself() {
    var snakeLocation = this.occupiedCoordinates();
    for(var i=0; i<snakeLocation.length-1; i++) {
      if (snakeLocation[i].x === this.head.x && snakeLocation[i].y === this.head.y) {
        return true;
      }
    }
    return false;
